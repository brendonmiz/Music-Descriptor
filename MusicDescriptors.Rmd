...
dataFilename2 <- 'DescMusJan5ed.csv'
...
col4excerpts <- ex.design$col4exgrp
ex.cols <- ex.design$ex.cols
...
a.000.zeMapCATA <- makeggHeatMap4CT(
                    CTstared(adj.cont.table,zeStars,'after'),
...
a.03adj.map4part <- BaseMap.Participants$zeMap_background + 
                  n.rv.means$zeMap_text + n.rv.means$zeMap_dots +
                    BaseMap.Participants$zeMap_dots + n.elliadj
...
```{r printadjpartwmeans, echo = TRUE, fig.height=10,figure.show = 'hold'}
a.03adj.map4part
...
ctradjI.1 <- PrettyBarPlot2(signed.ctrI.adj[,1],
                         ...
                         ylab = 'Contributions',
                         ylim = c(1.2*min(signed.ctrI.adj), 1.2*max(signed.ctrI.adj))
...
ctradjJ.1 <- PrettyBarPlot2(signed.ctrJ.adj[,1],
                         ...
                         font.size = 3,
                         color4bar = cols$adj.cols, # we need hex code
                         ylab = 'Contributions',
                         ylim = c(1.2*min(signed.ctrJ.adj), 1.2*max(signed.ctrJ.adj))
...
# plot contributions of rows for component 2
ctradjI.3 <- PrettyBarPlot2(signed.ctrI.adj[,3],
                         threshold = 1 / NROW(signed.ctrI.adj),
                         font.size = 3,
                         color4bar = cols$ex.oc, # we need hex code
                         ylab = 'Contributions',
...
# plot contributions of columns for component 2
ctradjJ.3 <- PrettyBarPlot2(signed.ctrJ.adj[,3],
                         threshold = 1 / NROW(signed.ctrJ.adj),
                         font.size = 3,
                         color4bar = cols$adj.cols, # we need hex code
                         ylab = 'Contributions',
